...
file_handler = logging.FileHandler("candlekeep_processing.log", mode="a", encoding="utf-8")
...
def clean_wikipedia_text(raw_text):
    ...
    # Step 7: Remove Wikipedia italics
...
def extract_sections(content):
    ...
    # Handle articles with no sections
...
# Step 4: Chunk the article with metadata
...
class ArticleBuffer:
    ...
    def add(self, chunks, title, edit_date):
        self.chunks.extend(chunks)
        self.titles.append(title)
        self.edit_dates.extend([edit_date] * len(chunks))
        self.total_chunks += len(chunks)
        
    def should_process(self):
        return len(self.chunks) >= self.buffer_size
...
def process_buffer(buffer, model_name, collection_name):
    if not buffer.chunks:
        return
        
    try:
        logger.info(f"Processing buffer with {len(buffer.chunks)} chunks from {len(buffer.titles)} articles")
        
        # Batch embed all chunks
        start_time = time.time()
        embeddings = embed_chunks(buffer.chunks, model_name)
        embed_time = time.time() - start_time
        logger.info(f"Embedding Time for {len(buffer.chunks)} chunks: {embed_time:.2f} seconds")
        
        # Batch insert into Milvus
        start_time = time.time()
        insert_into_milvus(buffer.chunks, embeddings, collection_name, buffer.edit_dates[0])
        insert_time = time.time() - start_time
        logger.info(f"Insert Time for {len(buffer.chunks)} chunks: {insert_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    # Initialize buffer
    buffer = ArticleBuffer(buffer_size=500)  # Adjust buffer size as needed
    ...
    try:
        for idx, (title, article_text, sha1, edit_date, templates, article_quality) in enumerate(stream_xml(args.file_path)):
            ...
            buffer.add(chunks, title, edit_date)
            
            # Process buffer when it's full
            if buffer.should_process():
                process_buffer(buffer, model_name, args.collection_name)
